...
<div class="sidebar">
        ...
        <ul class="optionlist view_more_ul">
            @if (isset($countryIdsArray) && count($countryIdsArray))
                @foreach ($countryIdsArray as $key => $country_id)
                    ...
                    @if (null !== $country)
                        @php
                            $checked = in_array($country->country_id, Request::get('country_id', []))
                                ? 'checked="checked"'
                                : '';
                        @endphp
                        <li>
                            <input type="checkbox" name="country_id[]" id="country_{{ $country->country_id }}"
                                value="{{ $country->country_id }}" {{ $checked }}>
                            <label for="country_{{ $country->country_id }}"></label>
                            {{ $country->country }}
                            <span>{{ App\User::countNumJobSeekers('country_id', $country->country_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
        ...
        <ul class="optionlist view_more_ul">
            @if (isset($stateIdsArray) && count($stateIdsArray))
                @foreach ($stateIdsArray as $key => $state_id)
                    ...
                    @if (null !== $state)
                        @php
                            $checked = in_array($state->state_id, Request::get('state_id', []))
                                ? 'checked="checked"'
                                : '';
                        @endphp
                        <li>
                            <input type="checkbox" name="state_id[]" id="state_{{ $state->state_id }}"
                                value="{{ $state->state_id }}" {{ $checked }}>
                            <label for="state_{{ $state->state_id }}"></label>
                            {{ $state->state }}
                            <span>{{ App\User::countNumJobSeekers('state_id', $state->state_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
                ...
                @foreach ($cityIdsArray as $key => $city_id)
                    ...
                    @if (null !== $city)
                        ...
                        <li>
                            ...
                            {{ $city->city }}
                            <span>{{ App\User::countNumJobSeekers('city_id', $city->city_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
                ...
                @foreach ($jobExperienceIdsArray as $key => $job_experience_id)
                    ...
                    @if (null !== $jobExperience)
                        ...
                        <li>
                            ...
                            <span>{{ App\User::countNumJobSeekers('job_experience_id', $jobExperience->job_experience_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
        ...
        <ul class="optionlist view_more_ul">
            @if (isset($careerLevelIdsArray) && count($careerLevelIdsArray))
                @foreach ($careerLevelIdsArray as $key => $career_level_id)
                    ...
                    @if (null !== $careerLevel)
                        @php
                            $checked = in_array($careerLevel->career_level_id, Request::get('career_level_id', []))
                                ? 'checked="checked"'
                                : '';
                        @endphp
                        <li>
                            ...
                            <span>{{ App\User::countNumJobSeekers('career_level_id', $careerLevel->career_level_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
        ...
        <ul class="optionlist view_more_ul">
            @if (isset($skillIdsArray) && count($skillIdsArray))
                @foreach ($skillIdsArray as $key => $job_skill_id)
                    ...
                    @if (null !== $jobSkill)
                        @php
                            $checked = in_array($jobSkill->job_skill_id, Request::get('job_skill_id', []))
                                ? 'checked="checked"'
                                : '';
                        @endphp
                        <li>
                            <input type="checkbox" name="job_skill_id[]" id="job_skill_{{ $jobSkill->job_skill_id }}"
                                value="{{ $jobSkill->job_skill_id }}" {{ $checked }}>
                            <label for="job_skill_{{ $jobSkill->job_skill_id }}"></label>
                            {{ $jobSkill->job_skill }}
                            <span>{{ App\User::countNumJobSeekers('job_skill_id', $jobSkill->job_skill_id) }}</span>
                        </li>
                    @endif
                @endforeach
            @endif
    ...
    {{-- <div class="widget">
            ...
            <div class="row">
                ...
                {!! Form::number('current_salary', Request::get('current_salary', null), array('class'=>'form-control', 'id'=>'current_salary', 'placeholder'=>__('From'))) !!}
                </div>
                </div>
                <div class="col-6">
                <div class="form-group">
                {!! Form::number('expected_salary', Request::get('expected_salary', null), array('class'=>'form-control', 'id'=>'expected_salary', 'placeholder'=>__('To'))) !!}
                </div>
                </div>
            </div>
